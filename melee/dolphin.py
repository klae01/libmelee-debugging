...
class Dolphin(Console):
    ...
    def __init__(self, ai_port, opponent_port, opponent_type, logger=None):
        ...
        if not os.path.exists(mem_watcher_path):
            ...
            print(
                "WARNING: Had to create a MemoryWatcher directory in Dolphin just now. "
    ...
    def get_dolphin_home_path():
        ...
        print(
            "ERROR: Are you sure Dolphin is installed? Make sure it is,\
                and then run again."
        )
        sys.exit(1)
        return ""
    ...
    def __update(self, mem_update, gamestate):
        ...
        if label == "frame":
            ...
            for i in gamestate.player:
                ...
                if (
                    abs(gamestate.player[i].x)
                    > stages.edgegroundposition(gamestate.stage)
                    or gamestate.player[i].y < -6
                ) and not gamestate.player[i].on_ground:
                ...
                gamestate.player[i].invulnerability_left = max(
                    0, gamestate.player[i].invulnerability_left - 1
                )
                ...
                if (
                    gamestate.player[i].action == Action.ON_HALO_DESCENT
                    and gamestate.frame > 150
                ):
                    gamestate.player[i].invulnerability_left = 120
                if (
                    gamestate.player[i].action == Action.EDGE_CATCHING
                    and gamestate.player[i].action_frame == 1
                ):
                ...
                if gamestate.player[i].transformed:
                    ...
                    gamestate.player[i].action_frame = gamestate.player[
                        i + 4
                    ].action_frame
                    gamestate.player[i].invulnerable = gamestate.player[
                        i + 4
                    ].invulnerable
                    ...
                    gamestate.player[i].speed_y_self = gamestate.player[
                        i + 4
                    ].speed_y_self
                    gamestate.player[i].speed_x_attack = gamestate.player[
                        i + 4
                    ].speed_x_attack
                    gamestate.player[i].speed_y_attack = gamestate.player[
                        i + 4
                    ].speed_y_attack
                    gamestate.player[i].speed_ground_x_self = gamestate.player[
                        i + 4
                    ].speed_ground_x_self
                ...
                if gamestate.player[i].action == Action.DASHING and gamestate.player[
                    i
                ].prev_action not in [Action.DASHING, Action.TURNING]:
        ...
        if label == "percent":
            if player_int > 4:
                try:
                    gamestate.player[player_int].percent = int(
            ...
            gamestate.player[player_int].percent = (
                gamestate.player[player_int].percent >> 16
            )
            return False
        if label == "stock":
            gamestate.player[player_int].stock = unpack("<I", mem_update[1])[0]
            gamestate.player[player_int].stock = (
                gamestate.player[player_int].stock >> 24
            )
            return False
        if label == "facing":
            gamestate.player[player_int].facing = unpack("<I", mem_update[1])[0]
            gamestate.player[player_int].facing = not bool(
                gamestate.player[player_int].facing >> 31
            )
            return False
        if label == "x":
        ...
        if label == "character":
            ...
            try:
                ...
                gamestate.player[player_int].character = (
        ...
        if label == "character_selected":
            ...
            try:
                ...
                gamestate.player[player_int].character_selected = (
                    enums.convertToInternalCharacterID(temp)
                )
            except ValueError:
                gamestate.player[player_int].character_selected = (
                    enums.Character.UNKNOWN_CHARACTER
        ...
        if label == "action":
            ...
            try:
                ...
                gamestate.player[player_int].prev_action = gamestate.player[
                    player_int
                ].action
        ...
        if label == "invulnerable":
            ...
            gamestate.player[player_int].invulnerable = (
                gamestate.player[player_int].invulnerable >> 31
            )
            return False
        ...
        if label == "jumps_left":
            ...
            try:
                totaljumps = int(
                    self.characterdata[gamestate.player[player_int].character]["Jumps"]
                )
        ...
        if label == "speed_air_x_self":
            gamestate.player[player_int].speed_air_x_self = unpack("<f", mem_update[1])[
                0
            ]
            return False
        ...
        if label == "speed_ground_x_self":
            gamestate.player[player_int].speed_ground_x_self = unpack(
                "<f", mem_update[1]
            )[0]
            return False
        if label == "coin_down":
        ...
        if label == "controller_status":
            temp = unpack(">I", mem_update[1])[0]
            temp = temp & 0x000000FF
            gamestate.player[player_int].controller_status = enums.ControllerStatus(
                temp
            )
            return False
        ...
        if label == "hitbox_4_y":
            ...
            return False
        if label == "iasa":
            gamestate.player[player_int].iasa = bool(
