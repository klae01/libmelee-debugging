    ...
    def update(self, mem_update):
        ...
        if label == "frame":
            ...
            for i in self.player:
                ...
                if (
                    abs(self.player[i].x) > stages.edgegroundposition(self.stage)
                    or self.player[i].y < -6
                ) and not self.player[i].on_ground:
        ...
        if label == "invulnerable":
            ...
            self.player[player_int].invulnerable = (
                self.player[player_int].invulnerable >> 31
            )
            return False
        ...
        if label == "jumps_left":
            ...
            try:
                totaljumps = int(
                    self.characterdata[self.player[player_int].character]["Jumps"]
                )
