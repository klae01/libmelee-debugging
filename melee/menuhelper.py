...
class MenuHelper:
    ...
    def menu_helper_simple(
        ...
        if gamestate.menu_state in [
            ...
            if gamestate.submenu == enums.SubMenu.NAME_ENTRY_SUBMENU:
                MenuHelper.name_tag_index = MenuHelper.enter_direct_code(
                    gamestate=gamestate,
        ...
        # If we're at the stage select screen, choose a stage
            ...
            if connect_code:
                MenuHelper.choose_direct_online(
                    gamestate=gamestate, controller=controller
                )
            else:
                MenuHelper.choose_versus_mode(
                    gamestate=gamestate, controller=controller
                )
    ...
    def choose_character(
        character, gamestate, port, controller, costume=2, swag=False, start=False
    ):
        ...
        if gamestate.menu_state == enums.Menu.SLIPPI_ONLINE_CSS:
            cursor_x, cursor_y = (
                gamestate.player[1].cursor_x,
                gamestate.player[1].cursor_y,
            )
            isSlippiCSS = True
