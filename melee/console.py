...
class Console:
            ...
            if EventType(event_bytes[0]) == EventType.PAYLOADS:
                ...
                try:
                    self._current_stage = enums.to_internal_stage(
                        unpack(">H", event_bytes[0x13 : 0x13 + 2])[0]
                    )
                except ValueError:
                    self._current_stage = enums.Stage.NO_STAGE
                event_bytes = event_bytes[event_size:]
                ...
                main_y = (main_y / 2) + 0.5
                gamestate.player[controller_port].controller_state.main_stick = (
                    main_x,
                    main_y,
                )
                ...
                gamestate.player[controller_port].controller_state.button[
                    enums.Button.BUTTON_A
                ] = bool(buttonbits & 0x0100)
                gamestate.player[controller_port].controller_state.button[
                    enums.Button.BUTTON_B
                ] = bool(buttonbits & 0x0200)
                gamestate.player[controller_port].controller_state.button[
                    enums.Button.BUTTON_X
                ] = bool(buttonbits & 0x0400)
                gamestate.player[controller_port].controller_state.button[
                    enums.Button.BUTTON_Y
                ] = bool(buttonbits & 0x0800)
                gamestate.player[controller_port].controller_state.button[
                    enums.Button.BUTTON_START
                ] = bool(buttonbits & 0x1000)
                gamestate.player[controller_port].controller_state.button[
                    enums.Button.BUTTON_Z
                ] = bool(buttonbits & 0x0010)
                gamestate.player[controller_port].controller_state.button[
                    enums.Button.BUTTON_R
                ] = bool(buttonbits & 0x0020)
                gamestate.player[controller_port].controller_state.button[
                    enums.Button.BUTTON_L
                ] = bool(buttonbits & 0x0040)
                gamestate.player[controller_port].controller_state.button[
                    enums.Button.BUTTON_D_LEFT
                ] = bool(buttonbits & 0x0001)
                gamestate.player[controller_port].controller_state.button[
                    enums.Button.BUTTON_D_RIGHT
                ] = bool(buttonbits & 0x0002)
                gamestate.player[controller_port].controller_state.button[
                    enums.Button.BUTTON_D_DOWN
                ] = bool(buttonbits & 0x0004)
                gamestate.player[controller_port].controller_state.button[
                    enums.Button.BUTTON_D_UP
                ] = bool(buttonbits & 0x0008)
                ...
                gamestate.player[controller_port].x = np.ndarray(
                    (1,), ">f", event_bytes, 0xA
                )[0]
                gamestate.player[controller_port].y = np.ndarray(
                    (1,), ">f", event_bytes, 0xE
                )[0]
                ...
                gamestate.player[controller_port].percent = int(
                    np.ndarray((1,), ">f", event_bytes, 0x16)[0]
                )
                gamestate.player[controller_port].shield_strength = np.ndarray(
                    (1,), ">f", event_bytes, 0x1A
                )[0]
                gamestate.player[controller_port].stock = np.ndarray(
                    (1,), ">B", event_bytes, 0x21
                )[0]
                ...
                try:
                    gamestate.player[controller_port].jumps_left = np.ndarray(
                        (1,), ">B", event_bytes, 0x32
                    )[0]
                except TypeError:
                ...
                try:
                    gamestate.player[controller_port].speed_air_x_self = np.ndarray(
                        (1,), ">f", event_bytes, 0x35
                    )[0]
                except TypeError:
                    gamestate.player[controller_port].speed_air_x_self = 0

                try:
                    gamestate.player[controller_port].speed_y_self = np.ndarray(
                        (1,), ">f", event_bytes, 0x39
                    )[0]
                except TypeError:
                    gamestate.player[controller_port].speed_y_self = 0

                try:
                    gamestate.player[controller_port].speed_x_attack = np.ndarray(
                        (1,), ">f", event_bytes, 0x2D
                    )[0]
                except TypeError:
                    gamestate.player[controller_port].speed_x_attack = 0

                try:
                    gamestate.player[controller_port].speed_y_attack = np.ndarray(
                        (1,), ">f", event_bytes, 0x41
                    )[0]
                except TypeError:
                    gamestate.player[controller_port].speed_y_attack = 0

                try:
                    gamestate.player[controller_port].speed_ground_x_self = np.ndarray(
                        (1,), ">f", event_bytes, 0x45
                    )[0]
                except TypeError:
                ...
                gamestate.player[controller_port].invulnerability_left = max(
                    0,
                    self._prev_gamestate.player[controller_port].invulnerability_left
                    - 1,
                )
                ...
                if (
                    gamestate.player[controller_port].action == Action.ON_HALO_DESCENT
                    and gamestate.frame > 150
                ):
                    gamestate.player[controller_port].invulnerability_left = 120
                if (
                    gamestate.player[controller_port].action == Action.EDGE_CATCHING
                    and gamestate.player[controller_port].action_frame == 1
                ):
                ...
                if gamestate.player[
                    controller_port
                ].action == Action.DASHING and self._prev_gamestate.player[
                    controller_port
                ].action not in [
                    Action.DASHING,
                    Action.TURNING,
    ...
    def __handle_slippstream_menu_event(self, event_bytes, gamestate):
        ...
        try:
            ...
            gamestate.player[1].controller_status = (
        ...
        try:
            ...
            gamestate.player[2].controller_status = (
                enums.ControllerStatus.CONTROLLER_UNPLUGGED
            )
        try:
            ...
            gamestate.player[3].controller_status = (
                enums.ControllerStatus.CONTROLLER_UNPLUGGED
            )
        try:
            ...
            gamestate.player[4].controller_status = (
                enums.ControllerStatus.CONTROLLER_UNPLUGGED
            )
        ...
        # Coin down
        try:
            gamestate.player[1].coin_down = (
                unpack(">B", event_bytes[0x2D : 0x2D + 1])[0] == 2
            )
        except error:
            gamestate.player[1].coin_down = False
        try:
            gamestate.player[2].coin_down = (
                unpack(">B", event_bytes[0x2E : 0x2E + 1])[0] == 2
            )
        except error:
            gamestate.player[2].coin_down = False
        try:
            gamestate.player[3].coin_down = (
                unpack(">B", event_bytes[0x2F : 0x2F + 1])[0] == 2
            )
        except error:
            gamestate.player[3].coin_down = False
        try:
            gamestate.player[4].coin_down = (
                unpack(">B", event_bytes[0x30 : 0x30 + 1])[0] == 2
            )
        except error:
    ...
    def _get_dolphin_home_path(self):
        ...
        print(
            "ERROR: Are you sure Dolphin is installed? Make sure it is, and then run again."
        )
        return ""
