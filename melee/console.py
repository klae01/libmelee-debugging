...
class Console:
    ...
    def __post_frame(self, gamestate, event_bytes):
        ...
        if controller_port in self._prev_gamestate.player:
            playerstate.invulnerability_left = max(
                0, self._prev_gamestate.player[controller_port].invulnerability_left - 1
            )
        ...
        if controller_port in self._prev_gamestate.player:
            if playerstate.action == Action.DASHING and self._prev_gamestate.player[
                controller_port
        ...
        try:
            if (
                abs(playerstate.x) > stages.EDGE_GROUND_POSITION[gamestate.stage]
                or playerstate.y < -6
    ...
    def __handle_slippstream_menu_event(self, event_bytes, gamestate):
        ...
        if gamestate.menu_state in [
            ...
            # Character selected
            try:
                gamestate.player[1].character_selected = enums.to_internal(
                    np.ndarray((1,), ">B", event_bytes, 0x29)[0]
                )
            except TypeError:
                gamestate.player[1].character_selected = (
                    enums.Character.UNKNOWN_CHARACTER
                )
            try:
                gamestate.player[2].character_selected = enums.to_internal(
                    np.ndarray((1,), ">B", event_bytes, 0x2A)[0]
                )
            except TypeError:
                gamestate.player[2].character_selected = (
                    enums.Character.UNKNOWN_CHARACTER
                )
            try:
                gamestate.player[3].character_selected = enums.to_internal(
                    np.ndarray((1,), ">B", event_bytes, 0x2B)[0]
                )
            except TypeError:
                gamestate.player[3].character_selected = (
                    enums.Character.UNKNOWN_CHARACTER
                )
            try:
                gamestate.player[4].character_selected = enums.to_internal(
                    np.ndarray((1,), ">B", event_bytes, 0x2C)[0]
                )
            except TypeError:
                gamestate.player[4].character_selected = (
                    enums.Character.UNKNOWN_CHARACTER
                )
        ...
        if gamestate.menu_state == enums.Menu.STAGE_SELECT:
            ...
            gamestate.stage_select_cursor_x = np.ndarray((1,), ">f", event_bytes, 0x31)[
                0
            ]
            gamestate.stage_select_cursor_y = np.ndarray((1,), ">f", event_bytes, 0x35)[
                0
            ]
        ...
        try:
            if gamestate.menu_state == enums.Menu.SLIPPI_ONLINE_CSS:
                for i in range(4):
                    gamestate.player[i + 1].costume = np.ndarray(
        ...
        try:
            for i in range(4):
                gamestate.player[i + 1].cpu_level = np.ndarray(
        ...
        try:
            for i in range(4):
                gamestate.player[i + 1].is_holding_cpu_slider = np.ndarray(
        ...
        for port in gamestate.player:
            if (
                gamestate.player[port].controller_status
    ...
    def get_dolphin_pipes_path(self, port):
        ...
        if platform.system() == "Linux":
            ...
            return (
                str(Path.home()) + "/.config/SlippiOnline/Pipes/slippibot" + str(port)
            )
