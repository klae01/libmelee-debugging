...
class FrameData:
    ...
    def isroll(self, character, action):
        for line in self.actiondata:
            if (
                line["action"] == str(action)
                and line["character"] == str(character)
    ...
    def inrange_simple(self, attacker, defender, future_frames=0):
        return self.inrange(
            attacker.character,
            attacker.action,
            attacker.action_frame + future_frames,
            attacker.facing,
            attacker.x,
            attacker.y,
            defender.character,
    ...
    def inrange(
        ...
        attackingframe = self.getframe(
            attacker_character, attacker_action, attacker_action_frame
        )
        if attackingframe is None:
            return False
        if (
            self.attackstate(attacker_character, attacker_action, attacker_action_frame)
            != enums.AttackState.ATTACKING
        ):
            return False
        ...
        distance1 = math.sqrt(
            (hitbox_1_x - defender_x) ** 2 + (hitbox_1_y - defender_y) ** 2
        )
        distance2 = math.sqrt(
            (hitbox_2_x - defender_x) ** 2 + (hitbox_2_y - defender_y) ** 2
        )
        distance3 = math.sqrt(
            (hitbox_3_x - defender_x) ** 2 + (hitbox_3_y - defender_y) ** 2
        )
        distance4 = math.sqrt(
            (hitbox_4_x - defender_x) ** 2 + (hitbox_4_y - defender_y) ** 2
        )
    ...
    def cleanupcsv(self):
        ...
        for row in self.rows:
            if (
                row["hitbox_1_status"] == True
                or row["hitbox_2_status"] == True
                or row["hitbox_3_status"] == True
                or row["hitbox_4_status"] == True
            ):
